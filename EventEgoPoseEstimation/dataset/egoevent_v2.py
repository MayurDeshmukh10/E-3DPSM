...
from EventEgoPoseEstimation.dataset.Joints3DDataset_v2 import Joints3DDatasetv2
...
from EventEgoPoseEstimation.dataset.egoevent_real_v2 import RealEventStreamv2
...
class SingleSequenceDataset(Joints3DDatasetv2):    
    def prepare_anno(self, item):
        ...
        if j3d is None or j2d is None:
            ...
            vis_j2d = np.ones_like(j2d) * valid_joints[:, None]
            vis_j3d = np.ones_like(j3d) * valid_joints[:, None]
    ...
    def __init__(self, cfg, data_path, is_train):
        ...
        if cfg.DATASET.TYPE == 'Synthetic':
            ...
            dataset = RealEventStreamv2(data_path, cfg, is_train)
...
class EgoEventv2(Dataset): 
    def __init__(self, cfg, temporal_bins, split, finetune=False):
        super().__init__()

        cfg = copy.deepcopy(cfg)
    
        if finetune and cfg.DATASET.FINETUNE_ON == 'Real':
            cfg.DATASET.TYPE = 'Real'
        elif finetune and cfg.DATASET.FINETUNE_ON == 'Wild':
            cfg.DATASET.TYPE = 'Wild'

        if cfg.DATASET.TYPE == 'Synthetic':
            dataset_root = Path(cfg.DATASET.SYN_ROOT)
        elif cfg.DATASET.TYPE == 'Real':
            dataset_root = Path(cfg.DATASET.REAL_ROOT)
        elif cfg.DATASET.TYPE == 'Wild':
            dataset_root = Path(cfg.DATASET.WILD_ROOT)
        else:
    ...
    def evaluate_joints(self, *args, **kwargs):
        return Joints3DDatasetv2.evaluate_joints(*args, **kwargs)

    @classmethod    
    def evaluate_dataset(cls, *args, **kwargs):
        return Joints3DDatasetv2.evaluate_dataset(*args, **kwargs)